...
(function () {
  ...
  var MODULES = [
    ...
    { id: "quiz", href: "ai-quiz.html", label: "AI出题", icon: "file-pen-line" },
    { id: "quest", href: "quest-trail.html", label: "答题闯关", icon: "route" }
  ];
  ...
  var QUIZ_BANK = [
    // 选择题
    { type: "choice", stem: "在渐进复杂度比较中，下列函数阶增长最慢的是？", options: ["O(n log n)", "O(n²)", "O(2ⁿ)", "O(n)"], answer: 3, explanation: "四个选项中 O(n) 增长最慢，其次为 O(n log n)、O(n²)、O(2ⁿ)。" },
    { type: "choice", stem: "下列关于大 O 记号的描述，正确的是？", options: ["描述算法最好情况", "描述算法平均情况", "描述算法上界（最坏情况）", "描述常数项"], answer: 2, explanation: "大 O 记号描述算法运行时间的渐进上界，通常关注最坏情况。" },
    { type: "choice", stem: "归并排序的渐进时间复杂度为？", options: ["O(n)", "O(n log n)", "O(n²)", "O(log n)"], answer: 1, explanation: "归并排序使用分治法，T(n)=2T(n/2)+O(n)，由主方法得 O(n log n)。" },
    { type: "choice", stem: "进程的基本状态不包括下列哪一项？", options: ["就绪", "运行", "阻塞", "编译"], answer: 3, explanation: "进程三态为就绪、运行、阻塞；编译不是进程状态。" },
    { type: "choice", stem: "下列哪种调度算法不会出现饥饿现象？", options: ["短作业优先", "高响应比优先", "时间片轮转", "优先级调度"], answer: 2, explanation: "时间片轮转让每个进程轮流执行，不会出现某进程长期得不到服务的情况。" },
    { type: "choice", stem: "TCP 三次握手过程中，第二次握手报文的标志位为？", options: ["SYN", "SYN+ACK", "ACK", "FIN"], answer: 1, explanation: "服务器收到 SYN 后回送 SYN+ACK，确认号 = 客户端序号 + 1。" },
    { type: "choice", stem: "在关系数据库中，下列哪项不是事务的 ACID 特性？", options: ["原子性", "一致性", "冗余性", "隔离性"], answer: 2, explanation: "ACID 指原子性、一致性、隔离性、持久性，冗余性不属于其中。" },
    { type: "choice", stem: "SQL 中用于去重的关键字是？", options: ["UNIQUE", "DISTINCT", "GROUP BY", "ORDER BY"], answer: 1, explanation: "DISTINCT 用于查询结果去重，作用于所有选中列。" },
    // 填空题
    { type: "fill", stem: "算法分析中，我们使用 ____ 记号描述运行时间的渐进上界。", answer: "大 O", explanation: "大 O 记号是渐进上界描述，记作 f(n) = O(g(n))。" },
    { type: "fill", stem: "分治法三步为：分解、____ 和合并。", answer: "求解/递归求解", explanation: "分治法的三步：分解为子问题、递归求解、合并结果。" },
    { type: "fill", stem: "进程的三种基本状态是就绪、____ 和阻塞。", answer: "运行", explanation: "三态模型：就绪、运行、阻塞。" },
    { type: "fill", stem: "TCP 建立连接需要 ____ 次握手，释放连接需要 ____ 次挥手。", answer: "三 / 四", explanation: "TCP 建连三次握手，断连四次挥手。" },
    { type: "fill", stem: "事务的四个特性 ACID 分别指原子性、____、隔离性、持久性。", answer: "一致性", explanation: "ACID = Atomicity、Consistency、Isolation、Durability。" },
    { type: "fill", stem: "在 SQL 中，____ 约束用于保证列值唯一且非空。", answer: "PRIMARY KEY", explanation: "PRIMARY KEY 主键约束默认唯一且非空。" },
    // 简答题
    { type: "short", stem: "简述大 O 记号在算法分析中的作用，并举例说明其比较意义。", answer: "大 O 记号用于描述算法在最坏情况下的渐进上界，忽略常数与低阶项。例如 O(n log n) 比相同输入下 O(n²) 增长更慢，因此在大规模数据上更优。", explanation: "应提及：上界含义、忽略常数、对比规模增长。" },
    { type: "short", stem: "什么是分治法？请举一个经典应用并说明其递归式。", answer: "分治法将问题分解为规模更小的子问题，递归求解后合并结果。典型应用：归并排序，递归式 T(n)=2T(n/2)+O(n)，由主方法得 O(n log n)。", explanation: "应涵盖：分解、递归、合并三步；举例正确；递归式正确。" },
    { type: "short", stem: "比较短作业优先与时间片轮转调度算法的优缺点。", answer: "短作业优先平均周转时间最优，但可能产生长作业饥饿；时间片轮转公平、响应快，无饥饿，但上下文切换开销大且平均周转时间不一定优。", explanation: "应指出 SJF 的饥饿问题与 RR 的公平性与切换开销。" },
    { type: "short", stem: "为什么 TCP 建立连接需要三次握手而不是两次？", answer: "三次握手是为了防止已失效的连接请求报文段突然到达服务器造成资源浪费。两次握手无法确认客户端的接收能力，也无法避免历史失效连接建立。", explanation: "关键点：失效连接请求、双向通道能力验证。" }
  ];
  ...
  function seedQuestions() {
    ...
    var sources = [
      { kind: "kp", label: "算法分析" },
      { kind: "chapter", label: "数据结构 · 第2章" },
      { kind: "text", label: "教师粘贴文本" },
      { kind: "doc", label: "算法导论.pdf" },
      { kind: "video", label: "分治法微课" }
    ];
    return QUIZ_BANK.slice(0, 10).map(function (q, i) {
      return {
        id: "seed-q" + (i + 1),
        type: q.type,
        stem: q.stem,
        ...
        source: sources[i % sources.length].kind,
        sourceLabel: sources[i % sources.length].label,
        createdAt: "08-30 09:" + pad2(10 + i)
      };
    });
  }
  ...
  function seedQuest() {
    return {
      devices: [
        { id: "wall",  name: "任务墙",   icon: "rectangle-vertical", color: "#147d6f", desc: "墙面投影题目，适合课堂集体闯关" },
        { id: "orb",   name: "知识光球", icon: "globe",             color: "#6a5fb0", desc: "悬浮光球点亮知识点，触摸解锁题目" },
        { id: "chest", name: "宝箱",     icon: "gift",              color: "#d99119", desc: "开启宝箱触发随机任务，奖励惊喜" },
        { id: "tv",    name: "互动电视", icon: "tv",                color: "#3e72ad", desc: "客厅式互动屏，多人协作答题" }
      ],
      zones: [
        {
          id: "z1", name: "算法基础区", device: "wall",
          questions: [
            { id: "zq1", type: "single", stem: "渐进复杂度 O(n log n) 比下列哪个增长更快？", options: ["O(n)", "O(log n)", "O(n²)", "O(1)"], answer: 2, explain: "四个选项中 O(n²) 比 O(n log n) 增长更快。" },
            { id: "zq2", type: "multi",  stem: "下列属于分治法典型应用的有？", options: ["归并排序", "快速排序", "Dijkstra", "Strassen 矩阵乘法"], answer: [0, 1, 3], explain: "归并、快排、Strassen 均为分治；Dijkstra 为贪心。" },
            { id: "zq3", type: "judge",  stem: "大 O 记号描述的是算法的最坏情况渐进上界。", answer: true, explain: "正确，大 O 描述渐进上界。" }
          ],
          unlocked: true, passed: false, best: 0
        },
        {
          id: "z2", name: "进程调度区", device: "orb",
          questions: [
            { id: "zq4", type: "single", stem: "时间片轮转调度的主要优点是？", options: ["平均周转时间最优", "无饥饿、响应快", "实现最简单", "适合长作业"], answer: 1, explain: "RR 公平、响应快、无饥饿。" },
            { id: "zq5", type: "judge",  stem: "短作业优先 (SJF) 不会出现饥饿。", answer: false, explain: "错误，SJF 可能让长作业长期等待。" },
            { id: "zq6", type: "multi",  stem: "下列属于进程基本状态的有？", options: ["就绪", "运行", "阻塞", "编译"], answer: [0, 1, 2], explain: "三态为就绪、运行、阻塞；编译非进程状态。" }
          ],
          unlocked: false, passed: false, best: 0
        },
        {
          id: "z3", name: "网络协议区", device: "chest",
          questions: [
            { id: "zq7", type: "single", stem: "TCP 第二次握手报文标志位是？", options: ["SYN", "SYN+ACK", "ACK", "FIN"], answer: 1, explain: "服务器回送 SYN+ACK。" },
            { id: "zq8", type: "judge",  stem: "TCP 释放连接需要四次挥手。", answer: true, explain: "正确，TCP 断连四次挥手。" },
            { id: "zq9", type: "multi",  stem: "下列属于传输层协议的有？", options: ["TCP", "UDP", "IP", "HTTP"], answer: [0, 1], explain: "TCP/UDP 为传输层；IP 是网络层；HTTP 是应用层。" }
          ],
          unlocked: false, passed: false, best: 0
        },
        {
          id: "z4", name: "数据库事务区", device: "tv",
          questions: [
            { id: "zq10", type: "single", stem: "SQL 中用于去重的关键字是？", options: ["UNIQUE", "DISTINCT", "GROUP BY", "ORDER BY"], answer: 1, explain: "DISTINCT 用于查询结果去重。" },
            { id: "zq11", type: "judge",  stem: "事务的 ACID 特性中 I 指隔离性。", answer: true, explain: "正确，Isolation。" },
            { id: "zq12", type: "multi",  stem: "下列属于事务 ACID 特性的有？", options: ["原子性", "一致性", "冗余性", "持久性"], answer: [0, 1, 3], explain: "冗余性不属于 ACID。" }
          ],
          unlocked: false, passed: false, best: 0
        }
      ],
      customBank: [] // 教师自定义题库（后台编辑时挑选入库）
    };
  }
  ...
  window.Platform = {
    ...
    modules: MODULES,
    Store: Store,
    fmt: { date: formatDate, clock: nowClock },
    quizBank: QUIZ_BANK,
