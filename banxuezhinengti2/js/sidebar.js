...
function renderSidebar() {
  ...
  KNOWLEDGE_TREE.forEach(folder => {
    ...
    folder.children.forEach(node => {
      ...
      const isAttempted = gameState.attemptedScenes.includes(node.sceneId);
      const isActive = gameState.currentScene === node.sceneId;
      const isLocked = !canAccessScene(node.sceneId);

      const nodeLi = document.createElement('li');
      nodeLi.className = `tree-node ${isCompleted ? 'completed' : ''} ${isAttempted && !isCompleted ? 'attempted' : ''} ${isActive ? 'active' : ''} ${isLocked ? 'locked' : ''}`;
