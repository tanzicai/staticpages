...
PageInits.microCourse = () => {
  ...
  // 如果是从学习任务跳转过来的，自动打开对应微课
  const pendingId = Store.get('pendingMicroCourseId');
  if (pendingId) {
    Store.set('pendingMicroCourseId', null); // 清除标记
    const targetMc = microCourses.find(m => m.id === pendingId);
    if (targetMc) {
      document.getElementById('microCourseDetail').style.display = 'block';
      currentMicroCourse = targetMc;
      targetMc.views++;
      Store.set('microCourses', microCourses);
      renderMicroCourseDetail(targetMc);
      // 滚动到详情区域
      setTimeout(() => {
        document.getElementById('microCourseDetail')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 100);
    }
  }
