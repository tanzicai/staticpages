...
(function (w) {
  ...
  function enrollModal(a, stu, refresh) {
    ...
    var pick = items.filter(function (it) {
      ...
      return !cap || U.count('enrollments', function (e) { return e.itemId === it.id && e.status !== '已驳回'; }) < cap;
    });
    ...
    function paintItems() {
      D.fill(itemHost, pick.map(function (it) {
        var cap = U.int(it.limit, 0);
        var used = U.count('enrollments', function (e) { return e.itemId === it.id && e.status !== '已驳回'; });
