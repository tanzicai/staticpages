...
(function (w) {
  ...
  /* ==========================================================================
     审核流程定义 —— 与活动实体上的 flowTpl 一一对应
     重要：这里的键必须覆盖「种子数据」与「活动创建表单」两处的全部取值，
     否则 flowOf() 会静默回落到默认流程，导致点一次「通过」后活动仍是待审核、
     活动广场看不到（数据流转断裂）。因此下方同时登记了命名与级数对应的全部模板。
     约定：第 1 项为「提交」节点（非审批），其余每项为一次审批；
           auditStage 表示当前待处理节点下标，推进到末项即发布。
     ====================================================================== */
  var FLOWS = {
    /* —— 活动创建表单（act.js）可选流程 —— */
    '无需审批（创建即发布）': [
      { n: '创建即发布', who: '活动组织者', role: 'organizer' }
    ],
    '一级审核（校团委）': [
      { n: '组织者提交', who: '活动组织者', role: 'organizer' },
      { n: '校团委审核', who: '校团委', role: 'auditor' }
    ],
    '二级审核（学院初审 + 校团委终审）': [
      { n: '组织者提交', who: '活动组织者', role: 'organizer' },
      { n: '院团总支初审', who: '二级学院团总支', role: 'college' },
      { n: '校团委终审', who: '校团委', role: 'auditor' }
    ],
    /* —— 种子数据与活动表单共用的流程 —— */
    '院级活动两级审核': [
      { n: '组织者提交', who: '活动组织者', role: 'organizer' },
    ...
    '活动申报（学生发起 → 学院初审 → 校团委终审）': [
      { n: '学生发起申报', who: '学生发起人', role: 'student' },
      { n: '指导学院初审', who: '二级学院团总支', role: 'college' },
      { n: '校团委终审', who: '校团委', role: 'auditor' }
    ]
  };
  /** 默认流程：单级审核。即使出现未登记的新流程名，也能在一次「通过」后正常发布 */
  var DEFAULT_FLOW = '一级审核（校团委）';
  function flowOf(a) { return FLOWS[(a || {}).flowTpl] || FLOWS[DEFAULT_FLOW]; }
  /** 该活动还需几次审批（用于界面明确提示，避免演示时误判“没生效”） */
  function remainAudits(a) {
    if (!a || a.status !== '待审核') return 0;
    var steps = flowOf(a);
    var stage = U.clamp(U.int(a.auditStage, 1), 0, steps.length - 1);
    return Math.max(1, steps.length - stage);
  }
  ...
  function auditOne(a, pass, opinion) {
    ...
    /* 发布后自动生成活动成立消息，便于在消息中心看到整条流转 */
    if (pass && last) {
      DB.insert('msgs', {
        title: '活动已进入活动广场',
        content: '「' + a.title + '」已完成全部审核，现已在活动广场对学生开放报名（' +
          ((a.items || []).length) + ' 个报名项目 · 认定 ' + U.num(a.credit) + ' 学分）。',
        type: '活动', scope: '全校', channels: ['站内消息'], at: U.dt(new Date()),
        sender: '系统', total: 1, readCount: 0, unreadCount: 1, readBy: [],
        bizType: '活动', bizId: a.id, needCall: false, remindWays: [], targetDesc: '全体学生'
      });
    }
    /* 返回剩余级数，供界面给出「还需 N 级」的明确反馈 */
    var after = DB.get('activities', a.id);
    return { to: to, last: last, remain: pass ? remainAudits(after) : 0, flowName: a.flowTpl || DEFAULT_FLOW };
  }
  ...
  function auditModal(a) {
    ...
    var m = UI.modal({
      title: '活动审核 · ' + a.title,
      sub: a.host + ' · ' + (a.flowTpl || DEFAULT_FLOW) + ' · 共 ' + (flowOf(a).length - 1) + ' 级审批，本活动还需 ' + remainAudits(a) + ' 级',
      size: 'xwide',
      body: host,
      foot: [
        ...
        D.h('button.btn.btn-sm.btn-p', {
          onclick: function () {
            ...
            if (mode === 'pass' && !pre.pass) {
              UI.confirm({
                ...
                onOk: function () {
                  ...
                  UI.toast('审核已通过', r.last ? ('「' + a.title + '」已进入活动广场，学生端现在可以报名')
                    : ('已推进至下一级审核（还需 ' + r.remain + ' 级），完成全部审批后自动进入活动广场'), 'ok');
                  m.close(); w.ZR.render();
                }
              });
              return;
            ...
            UI.toast(mode === 'pass' ? '审核已通过' : '已驳回',
              mode === 'pass'
                ? (r.last ? ('「' + a.title + '」已进入活动广场，学生端现在可以报名')
                  : ('已推进至下一级审核（还需 ' + r.remain + ' 级），完成全部审批后自动进入活动广场'))
                : '已通知活动组织者，活动状态回到「已驳回」', 'ok');
            m.close(); w.ZR.render();
          }
        }, mode === 'pass' ? '✓ 通过' : '✕ 驳回')
      ]
  ...
  function tabTodo(host) {
    ...
    host.appendChild(D.h('div.mt16', KP.lister({
      ...
      cols: [
        ...
        {
          t: '审核进度', w: 116, render: function (a) {
            var rm = remainAudits(a);
            if (!rm) return KP.status('已发布');
            return D.h('div', {}, KP.status('待审核'),
              D.h('div', { style: 'font-size:11px;color:var(--text3);margin-top:3px' }, '还需 ' + rm + ' 级'));
          }
        },
