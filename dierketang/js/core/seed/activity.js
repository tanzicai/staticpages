...
(function (w) {
  ...
  function build(rand, org) {
    ...
    catNames.forEach(function (catName, ci) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        activities.push({
          ...
          /* 创建时间必须早于当前时间（草稿/待审活动也一样，不能“未来创建”） */
        createdAt: (function () { var t = U.dt(U.toDate(start), false) + ' 09:00'; return (B.NOW && t > B.NOW) ? B.addDays(B.NOW, -3 - (seq % 20)) + ' 09:00' : t; })(),
    ...
    /* ---- 业务时间钳制 ----
       任何「已发生」的业务时间（报名、签到、作品提交、认定）都不能晚于系统当前时间，
       否则会出现「报名时间在未来」这类失真数据，也会让列表按时间倒序时把新数据挤到后面。 */
    var NOW_STR = String(B.NOW || '');   /* 种子统一时间基准，由 seed/base.js 提供 */
    function pastTime(dayStr, hh, mm) {
      var t = U.dt(dayStr, false) + ' ' + U.pad(hh) + ':' + U.pad(mm || 0);
      if (!NOW_STR || t <= NOW_STR) return t;
      /* 超出当前时间：回退到「当前时间前 1—30 天」的随机时刻。
         不同时区/日期解析差异可能让首轮回退仍晚于当前时间，故循环兜底直到确实落在过去。 */
      var back = 1 + Math.floor(rand() * 29);
      for (var guard = 0; guard < 6; guard++) {
        var d = new Date(NOW_STR.replace(/-/g, '/'));
        d.setDate(d.getDate() - back - guard);
        t = U.dt(d, false) + ' ' + U.pad(8 + Math.floor(rand() * 12)) + ':' + U.pad(Math.floor(rand() * 60));
        if (t <= NOW_STR) break;
      }
      return t;
    }
    ...
    activities.forEach(function (a) {
      ...
      a.items.forEach(function (it) {
        ...
        for (var t = 0; t < want * 3 && got < want; t++) {
          ...
          var eAt = pastTime(eDay, U.rndInt(rand, 8, 22), U.rndInt(rand, 0, 59));
      ...
      enrollments.filter(function (e) { return e.actId === a.id; }).forEach(function (e) {
        ...
        var sAt = pastTime(base, U.rndInt(rand, 13, 15), U.rndInt(rand, 0, 59));
      ...
      if (a.needWork && (a.status === '已结束' || a.status === '进行中' || a.status === '待开始')) {
        ...
        for (var i = 0; i < wn; i++) {
          ...
          works.push({
            ...
            at: pastTime(U.toDate(a.end), U.rndInt(rand, 9, 22), U.rndInt(rand, 0, 59)),
            status: st,
      ...
      if (a.status === '已结束') {
        enrollments.filter(function (e) {
          ...
          scoreRecs.push({
            ...
            at: pastTime(U.toDate(a.end), 18, 0),
    ...
    (function () {
      ...
      function addEnroll(stu, a, status, signStatus) {
        ...
        var e = {
          ...
          at: pastTime(d, U.rndInt(rand, 8, 22), U.rndInt(rand, 0, 59)),
          status: status,
        ...
        if (signStatus === '已签到') {
          signins.push({
            ...
            at: pastTime(U.toDate(a.start), 14, U.rndInt(rand, 0, 59)),
            status: '已签退',
            place: '校内网络校验通过',
            device: U.rndPick(rand, ['iOS 移动端', 'Android 移动端', '微信小程序']),
            makeUp: false
    ...
    apps.forEach(function (ap) {
      ...
      scoreRecs.push({
        ...
        at: (function () { var t = B.addDays(ap.at, 2); return (B.NOW && t > B.NOW) ? ap.at : t; })(), term: B.termOf(ap.at)
      });
    });
