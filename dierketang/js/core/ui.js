...
(function (w) {
  ...
  UI.table = function (opt) {
    ...
    /**
     * 返回值约定：**返回节点本身**，并把 tableEl / tbody / footEl / el 作为附加属性挂在节点上。
     * 原因：UI 其它组件（tabs / filterBar / steps / pager / card …）都返回节点，
     *      若本函数返回包装对象，调用方极易写出 `appendChild(UI.table(...))`，
     *      在真实浏览器中会抛 “appendChild 参数不是 Node”。
     *      el 指向自身，因此既支持 appendChild(UI.table(...))，也兼容旧的 .el 用法。
     */
    el.tableEl = table;
    el.tbody = tbody;
    el.footEl = footEl;
