...
function boot(opts) {
  ...
  try {
    w.DB.init(true);
    /* 默认以管理员登录；as='student' 时切换到「演示学生」身份，
       用于覆盖学生端（报名 / 签到 / 我的成绩单）的交互路径。
       重要：学生端交互此前未被扫描覆盖，曾遗漏 U.count 这类致命误调用。 */
    if (opts.as && w.ZDEMO && w.ZDEMO.loginAs) {
      const r = w.ZDEMO.loginAs(opts.as);
      sessionOk = !!(r && r.ok);
    }
    if (!sessionOk) {
  ...
  if (opts.withPages !== false && w.ZR) {
    try { w.ZR.start(opts.startAt || (w.ZA.isStudent() ? 'mine' : 'dash')); } catch (e) { errs.push({ file: 'router-start', msg: e.message }); }
  }
...
const rawOnly = process.argv[3] || '';
/* 支持 --as=student / --as=admin，用于切换验证身份 */
const asMatch = /^--as=([a-z]+)$/.exec(rawOnly);
const AS = asMatch ? asMatch[1] : '';
const only = asMatch ? '' : rawOnly;
...
if (mode === 'render' || mode === 'click') {
  const { w, errs, sessionOk } = boot({ as: AS });
  console.log('【装载】错误 ' + errs.length + (errs.length ? ' ' + JSON.stringify(errs) : '') +
    ' | 会话 ' + sessionOk + ' | 身份 ' + (AS || 'admin') + '(' + w.ZA.role() + ')');
  w.ZR.errors.length = 0;
