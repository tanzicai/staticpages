...
function loadSeedData() {
  ...
  [
    'js/core/util.js', 'js/core/dom.js',
    ...
    'js/core/seed/index.js'
  ].forEach(f => vm.runInContext(fs.readFileSync(path.join(ROOT, f), 'utf8'), pctx, { filename: f }));
  return probe.ZSEED.build();
}
...
/* --------------------------------------------------------------------------
   六、数据流转基础：业务时间合理性与命名空间误调用
   这两类问题都不会抛异常，但会直接让「演示数据流转」失效：
     ① 业务时间落在未来 → 按时间倒序的列表把新产生的记录挤到后面，演示时看不到；
     ② 页面调用了命名空间上不存在的函数（如 U.count）→ 关键按钮一点就抛错，
        整条流转链在该环节断裂（曾被验证盲区掩盖，因为只扫了管理端）。
   -------------------------------------------------------------------------- */
sec('六、数据流转基础（时间合理性 / 命名空间误调用）');
{
  /* ① 业务时间不得晚于系统当前时间 */
  const d2 = loadSeedData();
  const NOW = String(d2.meta.now || '');
  const TKEYS = ['at', 'createdAt', 'handledAt', 'learnedAt', 'joinedAt', 'submittedAt', 'reviewedAt'];
  const future = [];
  Object.keys(d2).forEach(k => {
    ...
    arr.forEach(x => {
      TKEYS.forEach(tk => {
        const v = String((x || {})[tk] || '');
        if (/^20\d\d-\d\d-\d\d/.test(v) && v > NOW) future.push(k + '.' + tk + '=' + v);
      });
    });
  });
  ok('业务时间均不晚于系统当前时间', future.length === 0,
    future.length ? future.length + ' 处，如 ' + future.slice(0, 3).join(' | ') : '基准 now=' + NOW);

  /* ② 页面不得调用命名空间上不存在的成员 */
  const probe2 = createWindow();
  const pctx2 = vm.createContext(probe2);
  const load3 = f => vm.runInContext(fs.readFileSync(path.join(ROOT, f), 'utf8'), pctx2, { filename: f });
  [
    'js/core/util.js', 'js/core/dom.js', 'js/core/ui.js', 'js/core/charts.js',
    'js/core/db.js', 'js/core/auth.js', 'js/core/router.js', 'js/core/qr.js',
    'js/core/demo.js', 'js/pages/_kit.js'
  ].forEach(load3);
  const ALIAS = { U: probe2.ZU, D: probe2.ZD, UI: probe2.ZUI, C: probe2.ZC, DB: probe2.DB, ZA: probe2.ZA, KP: probe2.ZKP, SG: probe2.ZSIGN };
  const pdir = path.join(ROOT, 'js/pages');
  const pfiles = fs.readdirSync(pdir).filter(f => f.endsWith('.js')).sort();
  const badCall = [];
  pfiles.forEach(f => {
    const src = fs.readFileSync(path.join(pdir, f), 'utf8');
    const used = {};
    Object.keys(ALIAS).forEach(a => {
      if (new RegExp('var ' + a + '\\s*=').test(src) && ALIAS[a]) used[a] = ALIAS[a];
    });
    src.split('\n').forEach((raw, i) => {
      const line = raw.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/, '');
      Object.keys(used).forEach(a => {
        ...
        let m;
        while ((m = re.exec(line))) {
          if (obj[m[1]] === undefined) badCall.push(f + ':' + (i + 1) + ' ' + a + '.' + m[1] + ' 不存在');
        }
      });
    });
  });
  ok('页面未调用命名空间上不存在的成员', badCall.length === 0,
    badCall.length ? [...new Set(badCall)].slice(0, 5).join(' | ') : '已比对 ' + pfiles.length + ' 个页面文件');
  console.log('  · 已覆盖：U / D / UI / C / DB / ZA / KP / SG 六个别名的全部调用');
}
